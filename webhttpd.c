...
static void send_template_ini_client(int client_socket, const char *template)
{
    ...
    if (nwrite != (ssize_t)(strlen(ok_response) + strlen(template)))
        MOTION_LOG(ERR, TYPE_STREAM, SHOW_ERRNO, "failure write");
}
...
static void send_template_raw(int client_socket, char *res)
{
    ...
    nwrite = write_nonblock(client_socket, res, strlen(res));
    if (nwrite < 0)
        MOTION_LOG(DBG, TYPE_STREAM, SHOW_ERRNO, "write_nonblock returned value less than zero.");
}
                                 ...
                                 unsigned int thread, int client_socket, void *userdata)
{
    ...
    if (cnt[0]->conf.webcontrol_parms == 0){
        sprintf(webparms,"%s","No Configuration Options");
    } else if (cnt[0]->conf.webcontrol_parms == 1){
        sprintf(webparms,"%s","Limited Configuration Options");
    } else if (cnt[0]->conf.webcontrol_parms == 2){
        sprintf(webparms,"%s","Advanced Configuration Options");
    } else{
        sprintf(webparms,"%s","Restricted Configuration Options");
    }
    ...
    if (!strcmp(command, "list")) {
        ...
        if (length_uri == 0) {
            ...
            if (cnt[0]->conf.webcontrol_html_output) {
                ...
                sprintf(res, "<a href=/%u/config>&lt;&ndash; back</a><br><br>\n<b>Camera %d%s%s</b><br></br>\n",
                        thread, cnt[thread]->conf.camera_id,
                        cnt[thread]->conf.camera_name ? " -- " : "",
                        cnt[thread]->conf.camera_name ? cnt[thread]->conf.camera_name : "");
                send_template(client_socket, res);

                sprintf(res, "webcontrol_parms = %d (%s)  <a href='%s#%s' target=_blank>help</a>\n\n<br><br><ul>"
                        ,cnt[0]->conf.webcontrol_parms, webparms, GUIDE_URL, "webcontrol_parms");
                send_template(client_socket, res);
                ...
                sprintf(res, "</ul><a href=/%u/config>&lt;&ndash; back</a>", thread);
                send_template(client_socket, res);
        ...
        if ((length_uri != 0) && (question == '?')) {
            ...
            if ((question == '=') && (command[0] != '\0')) {
                ...
                if (config_params[i].param_name) {
                    if (length_uri > 0) {
                        ...
                        if (cnt[0]->conf.webcontrol_html_output) {
                            sprintf(res,
                                "<a href=/%u/config/list>&lt;&ndash; back</a><br><br>\n"
                ...
                if ((cnt[0]->conf.webcontrol_html_output) && (command[0] != '\0') &&
                    ...
                    if (config_params[i].param_name) {
                        ...
                        if (!strcmp ("bool", config_type(&config_params[i]))) {
                            ...
                            sprintf(res, "<a href=/%u/config/list>&lt;&ndash; back</a><br><br>\n"
                                         "<b>Camera %d%s%s</b>\n"
                                         "<form action=set?>\n"
                                         "<b>%s</b>&nbsp;<select name='%s'>\n"
                                         "%s"
                                         ...
                                         cnt[thread]->conf.camera_name ? cnt[thread]->conf.camera_name : "",
                                         config_params[i].param_name, config_params[i].param_name,
                                         option, GUIDE_URL, config_params[i].param_name, text_help);
                        } else {
                            ...
                            sprintf(res, "<a href=/%u/config/list>&lt;&ndash; back</a><br><br>\n"
                                         ...
                                         cnt[thread]->conf.camera_name ? cnt[thread]->conf.camera_name : "",
                                         config_params[i].param_name, config_params[i].param_name,
                                         value, GUIDE_URL, config_params[i].param_name, text_help);
                        }
            ...
            if (cnt[0]->conf.webcontrol_html_output) {
                ...
                sprintf(res, "webcontrol_parms = %d (%s)  <a href='%s#%s' target=_blank>help</a><br></br>\n"
                        ,cnt[0]->conf.webcontrol_parms, webparms, GUIDE_URL, "webcontrol_parms");
                send_template(client_socket, res);

                sprintf(res, "<form name='n'>\n<select name='onames'>\n");
                send_template(client_socket, res);
                ...
                if (length_uri == 0) {
                    ...
                    if (config_params[i].param_name) {
                        ...
                        if (!strcmp(type, "unknown")) {
                            ...
                            if (cnt[0]->conf.webcontrol_html_output) {
                                send_template_ini_client(client_socket, ini_template);
                                sprintf(res, "<a href=/%u/config/get>&lt;&ndash; back</a><br><br>\n"
                                             "<b>Camera %d%s%s</b><br>\n<ul><li>%s = %s &nbsp;&nbsp;"
                                             ...
                                             cnt[thread]->conf.camera_name ? cnt[thread]->conf.camera_name : "",
                                             config_params[i].param_name, value,
                                             GUIDE_URL, config_params[i].param_name, text_help);
                                 ...
                                 unsigned int thread, int client_socket, void *userdata)
{
    ...
    if (!strcmp(command, "makemovie")) {
        ...
        if (length_uri == 0) {
            ...
            if (cnt[0]->conf.webcontrol_html_output) {
                send_template_ini_client(client_socket, ini_template);
                sprintf(res, "<a href=/%u/action>&lt;&ndash; back</a><br><br>\n"
        ...
        if (length_uri == 0) {
            ...
            if (thread == 0) {
                ...
                MOTION_LOG(NTC, TYPE_STREAM, NO_ERRNO, "httpd is going to restart thread %d",
                           thread);
        ...
        if (length_uri == 0) {
            ...
            if (thread == 0) {
                ...
                MOTION_LOG(NTC, TYPE_STREAM, NO_ERRNO, "httpd quits thread %d",
                           thread);
                                    ...
                                    unsigned int thread, int client_socket, void *userdata)
{
    ...
    if (!strcmp(command, "status")) {
        ...
        if (length_uri == 0) {
            ...
            if (cnt[0]->conf.webcontrol_html_output) {
                send_template_ini_client(client_socket, ini_template);
                sprintf(res, "<a href=/%u/detection>&lt;&ndash; back</a><br><br><b>Camera %d%s%s</b>"
                             " Detection status %s\n", thread, cnt[thread]->conf.camera_id,
                             cnt[thread]->conf.camera_name ? " -- " : "",
                             cnt[thread]->conf.camera_name ? cnt[thread]->conf.camera_name : "",
        ...
        if (length_uri == 0) {
            ...
            if (cnt[0]->conf.webcontrol_html_output) {
                send_template_ini_client(client_socket, ini_template);
                sprintf(res, "<a href=/%u/detection>&lt;&ndash; back</a><br><br>\n<b>Camera %d%s%s</b>"
                             " Detection resumed\n", thread, cnt[thread]->conf.camera_id,
                             cnt[thread]->conf.camera_name ? " -- " : "",
                             cnt[thread]->conf.camera_name ? cnt[thread]->conf.camera_name : "");
                send_template(client_socket, res);
                                ...
                                unsigned int thread, int client_socket, void *userdata)
{
    ...
    if (!strcmp(command, "set")) {
        ...
        if ((question == '?') && (length_uri > 2)) {
            ...
            if ((question != '=') || (command[0] == '\0')) {
                /* no valid syntax */
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 1");
                if (cnt[0]->conf.webcontrol_html_output)
                    response_client(client_socket, not_valid_syntax, NULL);
                else
                    response_client(client_socket, not_valid_syntax_raw, NULL);
            ...
            if (!strcmp(command, "pan")) {
                ...
                /* no valid syntax */
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 2");
                if (cnt[0]->conf.webcontrol_html_output)
                    response_client(client_socket, not_valid_syntax, NULL);
                else
                    response_client(client_socket, not_valid_syntax_raw, NULL);
            ...
            if (!warningkill) {
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 3");
                /* error value */
                if (cnt[0]->conf.webcontrol_html_output)
            ...
            if (length_uri == 0) {
                if (pan) {
                    ...
                    if (cnt[thread]->moved) {
                        if (cnt[0]->conf.webcontrol_html_output) {
                            send_template_ini_client(client_socket, ini_template);
                            sprintf(res, "<a href=/%u/track>&lt;&ndash; back</a><br><br>"
                                         "<b>Camera %d%s%s</b><br>\n"
            ...
            if ((question != '&') || (command[0] == '\0')) {
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 4");
                if (strstr(pointer, "&")) {
                    if (cnt[0]->conf.webcontrol_html_output)
                        response_client(client_socket, error_value, NULL);
                    else
            ...
            if (!strcmp(command, "pan")) {
                ...
                length_uri = length_uri - 3;
                if (pan || !tilt || X || Y) {
                    MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 5");
                    /* no valid syntax */
                    if (cnt[0]->conf.webcontrol_html_output)
                        response_client(client_socket, not_valid_syntax, NULL);
                    else
                ...
                if (tilt || !pan || X || Y) {
                    /* no valid syntax */
                    MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 6");
                    if (cnt[0]->conf.webcontrol_html_output)
                        response_client(client_socket, not_valid_syntax, NULL);
                    else
                        response_client(client_socket, not_valid_syntax_raw, NULL);
                ...
                length_uri--;
                if (X || !Y || pan || tilt) {
                    MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 7");
                ...
                length_uri--;
                if (Y || !X || pan || tilt) {
                    MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 8");
                    /* no valid syntax */
                    if (cnt[0]->conf.webcontrol_html_output)
                        response_client(client_socket, not_valid_syntax, NULL);
                    else
                ...
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 9");
                /* no valid syntax */
                if (cnt[0]->conf.webcontrol_html_output)
                    response_client(client_socket, not_valid_syntax, NULL);
                else
            ...
            if ((warningkill < 2) && (question != '=')) {
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 10");
                /* no valid syntax */
                if (cnt[0]->conf.webcontrol_html_output)
                    response_client(client_socket, not_valid_syntax, NULL);
                else
                ...
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 11");
                if (cnt[0]->conf.webcontrol_html_output)
                    response_client(client_socket, error_value, NULL);
                else
                    response_client(client_socket, error_value_raw, NULL);
            ...
            if (length_uri != 0) {
                MOTION_LOG(INF, TYPE_STREAM, NO_ERRNO, "httpd debug race 12");
                if (cnt[0]->conf.webcontrol_html_output)
                    response_client(client_socket, error_value, NULL);
                else
                    response_client(client_socket, error_value_raw, NULL);
            ...
            if (X && Y) {
                ...
                /* error in track pan */
                if (cnt[0]->conf.webcontrol_html_output) {
                    sprintf(res, "<a href=/%u/track>&lt;&ndash; back</a><br><br><b>Camera %d%s%s</b>\n",
                                 thread, cnt[thread]->conf.camera_id,
                                 cnt[thread]->conf.camera_name ? " -- " : "",
                                 cnt[thread]->conf.camera_name ? cnt[thread]->conf.camera_name : "");
                    response_client(client_socket, track_error, res);
                ...
                if (length_uri != 0) {
                    ...
                    if (!strcmp(command, "config")) {
                        ...
                        if (length_uri == 0) {
                            if (cnt[0]->conf.webcontrol_html_output) {
                                send_template_ini_client(client_socket, ini_template);
                                sprintf(res, "<a href=/%d/>&lt;&ndash; back</a><br><br>\n"
                                             "<b>Camera %d%s%s</b><br>\n"
                        ...
                        if (length_uri == 0) {
                            ...
                            if (cnt[thread]->track.type) {
                                ...
                                if (cnt[0]->conf.webcontrol_html_output) {
                                    sprintf(res, "<a href=/%d/>&lt;&ndash; back</a>\n", thread);
...
static unsigned int read_client(int client_socket, void *userdata, char *auth)
{
    ...
    while (alive) {
        ...
        if (nread <= 0) {
            MOTION_LOG(DBG, TYPE_STREAM, SHOW_ERRNO, "motion-httpd First Read Error");
            ...
            while ((strstr(buffer, "\r\n\r\n") == NULL) && (readb != 0) && (nread < length)) {
                ...
                if (nread > length) {
                    MOTION_LOG(WRN, TYPE_STREAM, SHOW_ERRNO, "motion-httpd End buffer"
             ...
             */
            if (nread == -1) {
                MOTION_LOG(ERR, TYPE_STREAM, SHOW_ERRNO, "motion-httpd READ give up!");
                pthread_mutex_unlock(&httpd_mutex);
                return 1;
            }
            alive = 0;
...
void httpd_run(struct context **cnt)
{
    ...
    MOTION_LOG(NTC, TYPE_STREAM, NO_ERRNO, "Started motion-httpd server on port %d (auth %s)",
    ...
    while ((client_sent_quit_message) && (!closehttpd)) {
        ...
        if (client_socket_fd < 0) {
            if ((!cnt[0]) || (cnt[0]->webcontrol_finish)) {
                MOTION_LOG(NTC, TYPE_STREAM, NO_ERRNO, "motion-httpd - Finishing");
                closehttpd = 1;
            }
        } else {
    ...
    free(authentication);
    close(sd);
    MOTION_LOG(NTC, TYPE_STREAM, NO_ERRNO, "motion-httpd Closing");
    pthread_mutex_destroy(&httpd_mutex);
}
...
void *motion_web_control(void *arg)
{
    ...
    MOTION_LOG(NTC, TYPE_STREAM, NO_ERRNO, "motion-httpd thread exit");
    pthread_exit(NULL);
}
